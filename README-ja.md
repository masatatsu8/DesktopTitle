...
5. **General** タブ — **Launch at login** (ログイン時の自動起動) と **Show for fullscreen apps** (フルスクリーン時のオーバーレイ) を切り替え、現在のディスプレイ構成のプロファイルモードを管理し、必要に応じて **Reset Current Profile to Defaults** で初期化します。
